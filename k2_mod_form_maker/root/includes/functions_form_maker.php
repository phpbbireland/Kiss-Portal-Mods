...
*
* Section of this code are taken form the appform mod by Sajaki
...
/*****
		...
		$sql = "SELECT id, form_id, ndx_order, name, hint, type, mandatory, options, forum_id
			FROM " . FORM_MAKER_TABLE . " AS m, " . FORUMS_TABLE . " AS f
			WHERE m.form_id = '" . $forum_id . "'
			AND m.form_id = f.forum_id
			ORDER BY ndx_order";
...
//		}
		...
		for ($i = 0; $i < $message_length; $i++)
		{
			...
			{
				$i = $i+ 2;
				while ($message[$i] != ':' && $i < $message_length)
				{
					$store1 .= $message[$i++];
				}
...
//		exit;
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			switch(strtolower($row['type']))
			{
				...
				case 'url':
				case 'inputbox':
					$entry = '<input style="border-radius: 5px;" class="text" style="width:300px;"
					value="'. $arr['D'][$j]. '"
					type="' . $row['type']. '" name="templatefield_' . $row['ndx_order'] . '"
					placeholder="' . $row['hint'] . '"
					size="40" maxlength="60" tabindex="' . $row['ndx_order'] . '" />';
				break;
				case 'textbox':
					$entry = '<textarea style="border-radius: 5px;" class="text" name="templatefield_' . $row['ndx_order'] . '" rows="3" cols="76"
					value="'. $arr['D'][$j]. '"
				...
				break;
				case 'selectbox':
					$entry = '<select style="border-radius: 5px;" class="inputbox" name="templatefield_' . $row['ndx_order'] . '" tabindex="' . $row['ndx_order'] . '">';
					$entry .= '<option value="">----------------</option>';
						 $select_option = explode(',', $row['options']);
						 foreach($select_option as $value)
						 {
				...
				case 'radiobuttons':
					...
					foreach($radio_option as $value)
					{
					   $entry .='<input type="radio" name="templatefield_'. $row['ndx_order'] .'" value="'. $value .'"/>&nbsp;'. $value .'&nbsp;&nbsp;';
					}
				break;
				case 'checkboxes':
					$check_option = explode(',', $row['options']);
					...
					foreach($check_option as $value)
					{
					   $entry .='<input type="checkbox" name="templatefield_'. $row['ndx_order'] .'[]" value="'. $value .'"/>&nbsp;'. $value .'&nbsp;&nbsp;';
					}
				break;
			}
