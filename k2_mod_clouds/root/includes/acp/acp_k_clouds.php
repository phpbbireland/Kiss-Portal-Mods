...
* @version $Id: acp_k_clouds.php 305 2009-01-01 16:03:23Z Michealo $
...
class acp_k_clouds
{
	...
	function main($id, $mode)
	{
		...
		$user->add_lang('acp/k_clouds');
		$this->tpl_name = 'acp_k_clouds';
		$this->page_title = 'ACP_K_CLOUDS';

		$form_key = 'acp_k_clouds';
		...
		switch ($action)
		{
			case 'config':
				...
				meta_refresh(1, "{$phpbb_root_path}adm/index.$phpEx$SID&amp;i=k_vars&amp;mode=config&amp;switch=k_clouds_vars.html");
			break;

			case 'add':
				$mode = '';
				meta_refresh(0, "{$phpbb_root_path}adm/index.$phpEx$SID&amp;i=k_clouds&amp;mode=add");
			break;
		...
		$template->assign_vars(array(
			'U_BACK'	=> "{$phpbb_root_path}adm/index.$phpEx$SID&amp;i=k_clouds",
			'U_ADD'		=> "{$phpbb_root_path}adm/index.$phpEx$SID&amp;i=k_clouds&amp;mode=add",
			'U_EDIT'	=> "{$phpbb_root_path}adm/index.$phpEx$SID&amp;i=k_clouds&amp;mode=edit" . '&amp;tag_id=' . $tag_id,
			'U_DELETE'	=> "{$phpbb_root_path}adm/index.$phpEx$SID&amp;i=k_clouds&amp;mode=delete" . '&amp;tag_id=' . $tag_id,
			'U_BROWSE'	=> "{$phpbb_root_path}adm/index.$phpEx$SID&amp;i=k_clouds&amp;mode=browse",
			'U_SWATCH1'	=> append_sid("{$phpbb_admin_path}swatch2.$phpEx", 'form=acp_k_clouds&amp;name=colour'),
			'U_SWATCH2'	=> append_sid("{$phpbb_admin_path}swatch2.$phpEx", 'form=acp_k_clouds&amp;name=colour2'),
			'U_SWATCH3'	=> append_sid("{$phpbb_admin_path}swatch2.$phpEx", 'form=acp_k_clouds&amp;name=hcolour'),
			'S_OPTION'	=> 'browse',
		));
		...
		switch ($mode)
		{
			case 'edit':
			{
				if ($submit)
				{
					...
					$is_active	= request_var('is_active', 1);
					$tag		= request_var('tag', 1);
					$link		= request_var('link', '');
					$rel		= request_var('rel', 'external');
					$font_size	= request_var('font_size', '10');
					...
					$sql = 'UPDATE ' . K_CLOUDS_TABLE . ' SET ' . $db->sql_build_array('UPDATE', $sql_ary) . " WHERE tag_id = $tag_id";
					...
					$cache->destroy('sql', K_CLOUDS_TABLE);
					...
					meta_refresh(1, "{$phpbb_root_path}adm/index.$phpEx$SID&amp;i=k_clouds&amp;mode=browse");
					break;
				}
			...
			case 'delete':
			{
				...
				if (confirm_box(true))
				{
					...
					*/
					...
					$sql = 'DELETE FROM ' . K_CLOUDS_TABLE . '
						WHERE tag_id = ' . $tag_id;
					$db->sql_query($sql);
			...
			case 'add':
			{
				if ($submit)
				{
	               ...
	               $sql_array = array(
					...
					$db->sql_query('INSERT INTO ' . K_CLOUDS_TABLE . ' ' . $db->sql_build_array('INSERT', $sql_array));
